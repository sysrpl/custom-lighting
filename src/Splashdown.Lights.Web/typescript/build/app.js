...
var Slider = (function () {
    ...
    Object.defineProperty(Slider.prototype, "method", {
        get: function () {
            return this._method;
        },
        set: function (value) {
    ...
    Object.defineProperty(Slider.prototype, "divisor", {
        get: function () {
...
function initControls() {
    ...
    var _loop_1 = function (i) {
        get("#color" + i).addEventListener("input", function (e) {
...
var commands = {
    select: function (select) {
        var program = get(select);
        if (isUndefined(program) || !program.hasClass("program"))
            return;
    ...
    settings: function () {
        ...
        if (selection) {
            ...
            sendWebRequest("/?method=getdetails", function (r) {
                ...
                get("#front").addClass("flip").removeClass("noflip");
                get("#back").addClass("flip").removeClass("noflip");
                get("#shadow").addClass("flip").removeClass("noflip");
                get("#edge").addClass("flip").removeClass("noflip");
...
function initPixels() {
    if (initPixels["__init"])
        return;
    initPixels["__init"] = true;
    var border;
    var colors = false;
    var pixels;
    function readPixels() {
    ...
    sendWebRequest("/?method=getborder", function (r) {
        ...
        for (var i = 0; i < pixels.length; i++) {
            ...
            if (i < border.bottom) {
                ...
                pixel.style.right = ((width - 10) / count * index) + "px";
            }
                ...
                pixel.style.top = "0";
                pixel.style.left = ((width - 12) / count * index) + "px";
            }
            else {
                index = i - border.top;
...
function main() {
    ...
    function effectsLoaded(effects) {
        var html = "";
    ...
    sendWebRequest("/?method=effects", function (r) { return effectsLoaded(r.responseJSON); });
    initControls();
    initPixels();
}
