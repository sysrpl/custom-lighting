...
{
	/// <summary>
	/// Wrapper class to controll Controller LEDs
	/// </summary>
	public class Controller : IDisposable
	{
        ...
        public void Render()
		{
			for(int i=0; i<= Settings.Channels.Length -1; i++)
			{
				if (Settings.Channels[i] != null)
				{
                    var data = Settings.Channels[i].Lights.Select(x => x.RGBValue);
                    if (Reverse)
                        data = data.Reverse();
                    if (Skip > 0) 
                        data = Enumerable.Repeat(0, Skip).Concat(data);
                    var ledColor = data.ToArray();
				    Marshal.Copy(ledColor, 0, _ws2811.channel[i].leds, ledColor.Length);
				}
			}
		...
		/// <param name="lightID">ID/Index of the LED</param>
		/// <param name="color">New color</param>
		public void SetLightColor(int channelIndex, int lightID, Color color)
		{
			Settings.Channels[channelIndex].Lights[lightID].Color = color;
		}
		...
		{
			_ws2811.channel[channelIndex].count			= channelSettings.Lights.Count;
